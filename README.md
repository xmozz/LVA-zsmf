# LVA-Zusammenfassungen

Sammlung meiner und [aldinapolis](https://github.com/ataldinapoli) Lernmaterialien verschiedenster LVAs

# 📁 Fächer

## [WS24](https://1drv.ms/o/c/1665f7f9d82b40c0/EgB861mOYxlLsFi-DPzkdkMBlMKRbD3oXta2Xbe2q61zuw?e=iLSHcs) - ADM, GDS EP1

- Zusammenfassungen für Fächer aus dem Wintersemester 2024 findest du in diesem [OneNote Notizbuch](https://1drv.ms/o/c/1665f7f9d82b40c0/EgB861mOYxlLsFi-DPzkdkMBlMKRbD3oXta2Xbe2q61zuw?e=iLSHcs) von mir und [aldinapoli](https://github.com/ataldinapoli).


---

## SS25

### [AlgoDat](AlgoDat) - Algorithmen und Datenstrukturen

- Zusammenfassung + Alttests

### [ANA](ANA) - Analysis 1

- Umfangreiche Formelsammlung für 1. und 2. Test von [aldinapoli](https://discord.com/users/575710766787985435).

### [DBS](dbs) – Datenbanksysteme

- Umfangreiche Ausarbeitung von den Slides.


### [EVC](EVC) – Computer Grafik & Computer Vision

- Umfangreiche Zusammenfassungen (vollständig & kapitelweise PDF), Formelsammlung, Alttests.


### [EP2](EP2) – Einführung in die Programmierung 2

- Strukturierte Zusammenfassungen.


---


## WS25

### [PP](PP) - Programmierparadigmen
- Zusammenfassung bestehend aus Skriptum und Slides.

### [SE](SE) - Software Engeneering
- UML-Zusammenfassung + Kapitelüberblick
- SE - Zusammenfassung + Alttests 

### [OS](OS) - Betriebssysteme
- Programmiertest 1 und 2 Zusammenfassung
- Übungs Code Walkthrough (in jeweiligen Zusammenfassungen (bis jetzt nur für Test1))
- Theorie Test Zusammenfassung

### [ThInf](ThInf) - Theoretische Informatik
- Zusammenfassung für die Prüfung

### [Statistik](Statistik) 
- Zusammenfassung für die Prüfung


---


## SS26

### [IntroAI](IntroAI) - Einführung in Artificial Intelligence
- Zusammenfassung für Prüfung

### [IntroSec](IntroSec) - Einführung in Security 
- Zusammenfassung für beide Tests

### [LaR](LaR) - Logic and Reasoning 
- Zusammenfassung für Prüfung

### [Multimedia](Multimedia) - Multimedia 
- Zusammenfassung für Prüfung
- Common Prüfungsfragen




## 📄 Format

- 1. Semester: [OneNote](http://onenote.cloud.microsoft/)
- 2. Semester + Aufwärts: [Obsidian](https://obsidian.md/)  

## 🙋 Feedback

Discord: [@xmozz](https://discord.com/users/409696362280517632), [@aldinapoli](https://discord.com/users/575710766787985435)


